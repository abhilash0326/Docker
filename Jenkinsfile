	...
	agent{
		...
		stages{
				...
				stage('making containers'){
									steps{
										sh "sudo docker run --name container1 -itdp httpd 80:70"
										sh "sudo docker run --name container2 -itdp httpd 80:90"
										sh "sudo docker run --name container3 -itdp httpd 80:8080"
